...
*   **Core Intents:**
    ...
    *   `CheckBalance`: Simulates checking account balance (uses Bedrock for EN responses, direct template for JA).
    *   `FollowupCheckBalance`: Handles follow-up balance checks using context.
    *   `MakeTransfer`: Simulates internal fund transfers with custom confirmation prompts (direct template for final success/cancel messages).
...
*   **Lambda Validation:** Implements date format validation for `CheckBalance` using a Lambda code hook.
...
4.  **Bedrock Model Access:**
    ...
    *   Request access to **Anthropic -> Claude Instant 1.2** (or another model if you prefer, but the Lambda code is currently tuned for Claude Instant's API).
...
*   **Configure EN (US):**
    *   Utterances: `check balance`, `check my {accountType} balance`.
    *   Slots:
...
*   **Configure EN (US):**
    ...
    *   **Confirmation:** **TURN OFF** the "Active" toggle (we will handle with Lambda).
    *   Save.
*   **Configure JA (JP):**
...
# ProBilingualBankerBot-Tokyo: A Lex V2 & Bedrock AI Chatbot
